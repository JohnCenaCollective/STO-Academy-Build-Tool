...
namespace Emzi0767.StoAcademyTools.Bot.Utilities
{
    public class BuildWriter : IDisposable
    {
        ...
        public void WriteBuild(StoAcademyBuild build, StoAcademyInterface tools)
        {
            ...
            var skills = this.ReduceSkills(build.Skills.Where(xsk => xsk.Region == SkillRegion.Space)).GroupBy(xsk => xsk.Rank);
            if (skills.Count() > 0)
            {
                this.MDW.WriteHeader("Space Skill Tree", 2);
                this.MDW.StartTable();
            ...
            var skillsg = this.ReduceSkills(build.Skills).Where(xsk => xsk.Region == SkillRegion.Ground).Select(xsk => new { s = xsk, i = int.Parse(xsk.SkillID.Substring(1)) });
            if (skillsg.Count() > 0)
            {
                this.MDW.WriteHeader("Ground Skill Tree", 2);
                this.MDW.StartTable();
                this.MDW.WriteTableHeaders(new string[] { "Skill", "Skill" });
                this.MDW.WriteTableAlignments(new string[] { "center", "center" });
                var sl1 = skillsg.Where(xski => xski.i < 7);
                var sl2 = skillsg.Where(xski => xski.i > 12 && xski.i < 17);
                var sr1 = skillsg.Where(xski => xski.i > 6 && xski.i < 13);
                var sr2 = skillsg.Where(xski => xski.i > 16);
                ...
                var unlocks = build.SkillUnlocks.Where(xsku => xsku.Career == SkillUnlockCareer.Unknown).OrderBy(xsku => xsku.UnlockID);
                if (unlocks.Count() > 0)
                {
            ...
            if (!string.IsNullOrWhiteSpace(build.Ship.Name))
            {
                ...
                var scomponents = build.BuildItems.OrderBy(xbi => xbi.ItemType);
                if (scomponents.Count() > 0)
                {
                    ...
                    foreach (var item in scomponents)
                    {
                        this.MDW.StartTableRow();
            ...
            if ((build.BOFFs.Count() > 0 && build.BOFFs.SelectMany(xbo => xbo.Abilities).Count() > 0) || 
                build.DOFFs.Where(xdo => xdo.DOFF.Region == DoffRegion.Space).Count() > 0 || 
                (build.AwayTeam.Count() > 0 && build.AwayTeam.SelectMany(xbo => xbo.Abilities).Count() > 0) || 
                build.DOFFs.Where(xdo => xdo.DOFF.Region == DoffRegion.Ground).Count() > 0)
            {
                ...
                {
                    this.MDW.StartTable();
                    this.MDW.WriteTableHeaders(new string[] { "Space Duty Officers", "Power" });
                    this.MDW.WriteTableAlignments(new string[] { "left", "left" });
                ...
                {
                    this.MDW.StartTable();
                    this.MDW.WriteTableHeaders(new string[] { "Ground Duty Officers", "Power" });
                    this.MDW.WriteTableAlignments(new string[] { "left", "left" });
            ...
            this.MDW.WriteParagraph("*I am a robit. Above was translated automatically using [Automatic Build Converter](https://github.com/JohnCenaCollective/STO-Academy-Build-Tool), created by Emzi0767, maintained by JohnCenaPTF.*");
