...
converted template still lacks some information. 
...
Please don't alter the output from the application. 
...
Additionally, always remember to fill out as much 
data as possible, so that we can provide assistance 
...
* Windows:
   * Minimum required Windows version: Windows Vista  
     Service Pack 2
...
* GNU/Linux, Mac OS, and other *NIX systems:
   ...
   * Minimum required Mono version is 3.2.x, however  
     I will only provide support for 4.x
...
2. Run abt.exe and wait for the program to load its 
   data
3. Enter the build ID into the text box
   Build ID is the part after 
...
1. Open Terminal
2. Navigate to the directory where you extracted the 
   converter  
   cd path
   * For example, if it was extracted to a directory 
     called abt inside your Downloads directory, you 
	 want to 
     cd Downloads/abt
